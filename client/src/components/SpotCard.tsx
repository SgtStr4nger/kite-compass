...
export function SpotCard({
  ...
  const href = month ? `/spots/${spot.slug}?month=${month}` : `/spots/${spot.slug}`;
  return (
    <Link
      href={href}
        ...
        <div className="flex flex-1 flex-col gap-2 p-4">
          ...
          {rec && (
            <div className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs text-foreground/80">
              ...
              {rec.averageBaseWind != null && (
                <span className="inline-flex items-center gap-1"><Wind className="h-3.5 w-3.5 text-primary" />{rec.averageBaseWind} kn avg</span>
              )}
              {rec.gusts != null && (
                <span className="inline-flex items-center gap-1"><Gauge className="h-3.5 w-3.5 text-primary" />{rec.gusts} kn gusts</span>
              )}
              {rec.windDays != null && (
                <span className="inline-flex items-center gap-1"><CalendarDays className="h-3.5 w-3.5 text-primary" />{rec.windDays} wind days</span>
              )}
            </div>
          )}
